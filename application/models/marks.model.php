...
class marks extends table
{
  ...
  {
    # With feed
    $query = $this->query_latest_ids_and_ts;
    return helper('feed')->marks("feed_marks", $query);
    # With search
    # return helper('search')->search();
  }

  function with_tag($tag)
  {
    # With feed
    $query = $this->query_ids_and_ts_with_tag->__use($tag);
    return helper('feed')->marks("feed_marks_tag_{$tag->id}", $query);
    # With search
  ...
  {
    # With feed
    $query = $this->query_ids_and_ts_from_user->__use($user);
    return helper('feed')->marks("feed_marks_user_{$user->id}", $query);
    # With search
    # return helper('search')->search(['user' => $user]);
  }

  function from_user_with_tag($user, $tag)
  {
    # Only available with search helper
    return helper('search')->search(['user' => $user, 'tag' => $tag]);
  }

  function private_from_user($user)
  {
    # With feed
    $query = $this->query_ids_and_ts_from_user->__use($user, ['private' => true]);
    return helper('feed')->marks("feed_marks_my_{$user->id}", $query);
    # With search
    # return helper('search')->search(['user' => $user, 'private' => true]);
  }

  function private_from_user_with_tag($user, $tag)
  {
    # Feed
    $query = $this->query_ids_and_ts_from_user_with_tag->__use($user, $tag, ['private' => true]);
    return helper('feed')->marks("feed_marks_my_{$user->id}_tag_{$tag->id}", $query);
    # With search
    # return helper('search')->search(['user' => $user, 'tag' => $tag, 'private' => true]);
  }

  function private_from_user_with_tags($user, $tags)
  {
    # Only available with search helper
